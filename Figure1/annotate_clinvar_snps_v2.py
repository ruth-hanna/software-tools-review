...
*****
Prior to using this script, you must download the chromosome files for GRCh38 from here:
http://hgdownload.soe.ucsc.edu/goldenPath/hg38/chromosomes/

Specify the path to the folder containing these files using --path.
*****
...
# Specifies the number of nucleotides to extract on each side of SNP
BUFFER_LEN = 50
...
def GetParser():
    ...
    parser.add_argument('--sgrna_type',
    	...
    	help='edit_in designs sgRNAs to introduce ClinVar SNPs (i.e. sgRNA targets WT sequence), edit_out designs sgRNAs to correct ClinVar SNPs (i.e. sgRNA targets mutant sequence)')
    ...
    parser.add_argument('--path',
    	type=str,
    	help='location of fasta files for GRCh38')
    return parser
...
def ReadChromFiles(path,start_chrom,end_chrom):
	...
	for chrom_num in range(start_chrom,end_chrom+1):
		...
		if chrom_num == 24:
			chrom_num = 'Y'
		chr_file = path + 'chr' + str(chrom_num) + '.fa.gz'
...
def LocateSNP(chr_dict, parsed_variant_df, sgrna_type):
	...
	for index, row in parsed_variant_df.iterrows():
		...
		preceding_genomic_context = snp_chr_seq[(snp_position-BUFFER_LEN-1):snp_position-1].upper()
		succeeding_genomic_context = snp_chr_seq[(snp_position):(snp_position+BUFFER_LEN)].upper()
	...
	print 'Number of cases where reference alleles do not match = ' ref_allele_mismatch
	#print error_list
	if error_list:
		snp_error_df = pd.DataFrame(data=error_list,columns = snp_df_columns_list)
	else:
		snp_error_df = pd.DataFrame(columns = snp_df_columns_list)
...
def main():
	...
	# Read in chrom files.
	chr_dict = ReadChromFiles(args.path,1,24)
	# Locate SNPs.
